...
const Dashboard = () => {
  ...
  const loadGenes = async () => {
    try {
      ...
      setAvailableGenes(response.data.genes);
    } catch (error) {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      const response = await axios.post(`${API}/upload-sequence`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      setSequence(response.data.sequence);
      toast.success(`Loaded sequence (${response.data.length} nucleotides)`);
    } catch (error) {
